...
* remove text justification hack (#92) (PR sent)
* warn in case when target of inter-document xref can't be resolved, but xref has explicit text
* don't add image from front matter if already referenced by chapter
* add document filename to warn messages
...
* support inline SVG
* describe the leveloffset setting in more detail
...
* MEDIUM: add icon & icon placement as attributes on btn macro (was auto-space button macro, whatever that means)
...
* namo pubtree is justifying preformatted blocks
