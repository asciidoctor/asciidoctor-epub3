...
:idprefix:
:idseparator: -
ifndef::imagesdir[:imagesdir: data/samples/images]
...
{project-name} takes an aggregate AsciiDoc document and any assets it references as input and produces to an EPUB3 publication archive (often described as a “website in a box”).
...
For more information about this attribute and other related attributes, see http://asciidoctor.org/docs/user-manual/#builtin-attributes-i18n[internationalization and numbering].
...
https://www.amazon.com/gp/sendtokindle/[“Send to Kindle”] is a tempting choice for transferring MOBI files to a Kindle device.
...
Whispersync will detect the new file (usually looking in the Books folder) and add it to the “On Device” library.
