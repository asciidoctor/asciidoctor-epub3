...
Images referenced in your AsciiDoc document should be stored in the directory defined by the `imagesdir` attribute.
We term this location the images catalog.
...
Many of the EPUB3 ereaders use the http://webkit.org[WebKit browser engine] to render the content and apply the CSS formatting and styles.
Generally speaking, WebKit is a great engine that brings a lot of consistency and power to the ereading landscape.
