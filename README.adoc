...
:project-uri: https://github.com/opendevise/{project-handle}
:project-repo-uri: https://github.com/opendevise/{project-handle}
...
Fully Semantic::
  Produce highly semantic XHTML documents, including use of the recommended +epub:type+ attribute.
Optimal Readability::
...
If you want to clone the git repository, simply copy the {project-repo-uri}[GitHub repository URL] and pass it to the +git clone+ command:
 ...
 $ git clone https://github.com/opendevise/asciidoctor-epub3
...
The term [term]_package metadata_ in Table 1 is in reference to the http://www.idpf.org/epub/30/spec/epub30-publications.html#sec-metadata-elem[metadata element in the EPUB3 package document, named [file]_package.opf_].
The +dc+ prefix is in reference to the http://dublincore.org/documents/2004/12/20/dces[Dublin Core Metadata Element Set].
...
TIP: As another example, point the +asciidoctor-epub3+ command at the https://github.com/opendevise/github-guides-asciidoc[GitHub Guides] that we've ported to AsciiDoc.
...
.EPUB3 with validation
 $ asciidoctor-epub3 -D output --validate data/samples/sample-book.adoc
...
If you want to browse the contents of the EPUB3 file that is generated, or preview the XHTML files in a regular web browser, add the +--extract+ flag to the +asciidoctor-epub3+ command.
The EPUB3 file will be extracted to a directory adjacent to the generated file, but without the file extension.

 $ asciidoctor-epub3 -D output --extract data/samples/sample-book.adoc
...
You just need to specify the format (+-f+) as +kf8+.

 $ asciidoctor-epub3 -D output -f kf8 data/samples/sample-book.adoc
...
Reflecting on the problem of the zero-width space brings us to the word joiner character (e.g., `<a href="...">Fork</a>&#8288;`).
Like the zero-width space, the word joiner occupies no space.
However, instead of introducing a wrap opportunity, it prevents one.
But here's the clincher.
If the character following the word joiner is a normal space (e.g., `<a href="...">Fork</a>&#8288; the`), then it acts as though the word joiner is not present.
All the scenarios are accounted for!
WebKit, you've been Unicode punked!

_By adding the word joiner character immediately after any inline element, we can trick WebKit into justifying the text properly, as shown in the second screenshot above._
...
{project-name} was written by https://github.com/mojavelinux[Dan Allen] and https://github.com/graphitefriction[Sarah White] of OpenDevise Inc.
